...
export const TunnelHistoryPage: React.FC = () => {
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const data = await apiRequest<any>("/v1/tunnel/sessions?status=active,pending,disconnected,revoked");
  ...
  const filtered = useMemo(() => {
    return sessions.filter((s) => {
      const matchesSearch = !search || s.subdomain.toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "2rem" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            ...
            <h1 style={{ fontSize: "1.75rem", fontWeight: 700 }}>Tunnel History</h1>
          </div>
          <p style={{ color: "#94a3b8" }}>All past and current tunnel sessions with timestamps and durations.</p>
        </div>
      ...
      {loading ? (
        <div style={{ textAlign: "center", padding: "3rem", color: "#64748b" }}>Loading tunnel history...</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "3rem", color: "#64748b" }}>
          <History size={40} style={{ margin: "0 auto 1rem" }} />
          <p>No tunnel sessions found matching your filters.</p>
        </div>
      ) : (
            ...
            <tbody>
              {filtered.map((s) => (
                <tr key={s.id} style={{ borderBottom: "1px solid #0f172a" }}>
                  ...
                  <td style={{ padding: "10px 12px" }}>
                    {s.public_url ? (
                      <a href={s.public_url} target="_blank" rel="noreferrer" style={{ color: "#6366f1", display: "flex", alignItems: "center", gap: 4 }}>
